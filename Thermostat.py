...
class TemperatureMachine(StateMachine):
    ...
    def updateLights(self):
        ...
        try:
            redLight.source = None
        except Exception:
            pass

        try:
            blueLight.source = None
        except Exception:
            pass
        ...
        if self.current_state == self.heat:
            if temp < self.setPoint: 
                # Red light fade in and out
                redLight.pulse(fade_in_time=1.0, fade_out_time=1.0, n=None, background=True)
                blueLight.off()
            else:
                # Solid red light
                redLight.on()
            ...
            if temp > self.setPoint:
                # Blue light fade in and out
                blueLight.pulse(fade_in_time=1.0, fade_out_time=1.0, n=None, background=True)
                redLight.off()
            else:
                # Solid blue light
                blueLight.on()
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            ## Remove this TODO comment block when complete.
            line1 = current_time.strftime("%Y-%m-%d %H:%M:%S")
            ...
            if(altCounter < 6):
                ...
                state_str = self.current_state.id.upper()
                line2 = f"{state_str} SPoint: {self.setPoint}F"
                line2 = (line2[:16]).ljust(16)
                lcd_line_2 = line2
            ...
            if((counter % 30) == 0):
                ...
                ## Remove this TODO comment block when complete.
                payload = self.setupSerialOutput() + "\n"
